...
void iris_cm_color_gamut_set(u32 level, bool bcommit)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7)
		...
		iris_cm_color_gamut_set_i8(level, bcommit);
}
...
void iris_pwil_dport_disable(bool enable, u32 value)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7)
		...
		iris_pwil_dport_disable_i8(enable, value);
}
		...
		size_t count, loff_t *ppos)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7)
		...
		iris_configure_i8(DSI_PRIMARY, type, value);
	...
	for (i = 0; i < len; i++) {
		if (pcfg->iris_chip_type == CHIP_IRIS7)
			...
			iris_configure_get_i8(DSI_PRIMARY, arr[i].type, 1, &cfg_val);
...
int iris_update_backlight(u32 bl_lvl)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7)
		ret = iris_update_backlight_i7(bl_lvl);
	else if (pcfg->iris_chip_type == CHIP_IRIS7P)
		ret = iris_update_backlight_i7p(bl_lvl);
	else if (pcfg->iris_chip_type == CHIP_IRIS8)
		ret = iris_update_backlight_i8(bl_lvl);
